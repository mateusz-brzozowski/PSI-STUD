...
class Sender:
    ...
    def send_session_data(self) -> None:
        self._type = 1
        control_sum = self._public_key + self._primitive_root + self._prime_number % 2 ** 16
        print(
            f"{self._type}:{control_sum}:{self._public_key}:{self._primitive_root}:{self._prime_number}")
        print(f"{self._type:03b}{control_sum:016b}{self._public_key:064b}{self._primitive_root:032b}{self._prime_number:032b}")
